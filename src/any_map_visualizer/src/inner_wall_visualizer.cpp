...
int main(int argc, char** argv) {
    ros::init(argc, argv, "inner_wall_visualizer");
    ros::NodeHandle nh;

    // CSV 파일 경로 설정
    std::string csv_file;
    nh.param<std::string>("csv_file", csv_file, INNER_WALL_CSV_PATH);

    std::vector<std::vector<double>> centerline;
    readCSV(csv_file, centerline);

    ros::Publisher marker_pub = nh.advertise<visualization_msgs::Marker>("/visual_inner_wall", 1);
    ros::Rate r(1);
