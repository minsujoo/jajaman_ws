...
int main(int argc, char** argv) {
    ros::init(argc, argv, "centerline_10_visualizer");
    ros::NodeHandle nh;

    // CSV 파일 경로 설정
    std::string csv_file;
    nh.param<std::string>("csv_file", csv_file, CENTERLINE_CSV_PATH);

    std::vector<std::vector<double>> centerline;
    readCSV(csv_file, centerline);

    ros::Publisher marker_pub = nh.advertise<visualization_msgs::Marker>("/visual_centerline_10", 1);
    ros::Rate r(1);
